...
func generateMainWithDomains(artifactsDir string, serviceFuncs []ssacparser.ServiceFunc, modulePath string, queueBackend string, policies []*policy.Policy, sessionBackend, cacheBackend string, fileConfig *manifest.FileBackend) error {
	if modulePath == "" {
