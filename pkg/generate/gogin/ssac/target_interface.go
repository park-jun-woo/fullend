...
type Target interface {
	...
	GenerateFunc(sf ssacparser.ServiceFunc, st *validator.SymbolTable) ([]byte, error)

	// GenerateModelInterfaces는 서비스 함수에서 사용하는 모델 인터페이스를 생성한다.
	GenerateModelInterfaces(funcs []ssacparser.ServiceFunc, st *validator.SymbolTable, outDir string) error

	// GenerateHandlerStruct는 도메인별 Handler struct를 생성한다.
	GenerateHandlerStruct(funcs []ssacparser.ServiceFunc, st *validator.SymbolTable, outDir string) error
