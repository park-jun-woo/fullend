...
func TestGenerateModelInterfaceQueryOptsExplicit(t *testing.T) {
	...
	funcs := []ssacparser.ServiceFunc{{
		...
		Sequences: []ssacparser.Sequence{
			{Type: ssacparser.SeqGet, Model: "User.FindByID", Inputs: map[string]string{"ID": "currentUser.ID"}, Result: &ssacparser.Result{Type: "User", Var: "user"}},
			{Type: ssacparser.SeqGet, Model: "Reservation.ListByUserID", Inputs: map[string]string{"UserID": "currentUser.ID", "Opts": "query"}, Result: &ssacparser.Result{Type: "[]Reservation", Var: "reservations"}},
			{Type: ssacparser.SeqResponse, Fields: map[string]string{"reservations": "reservations"}},
		},
	}}
