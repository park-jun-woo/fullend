...
목표:
  ...
  커스텀 없으면 → go get github.com/park-jun-woo/fullend → import하여 사용
...
프로젝트 `go.mod`에서:
```
go get github.com/park-jun-woo/fullend
```

생성된 코드에서:
```go
import auth "github.com/park-jun-woo/fullend/pkg/auth"
...
로컬 vs 외부 판별:
- import 경로가 `github.com/park-jun-woo/fullend/pkg/` → fullend 기본 제공 (검증 스킵, go get으로 해결)
...
genFunc():
  ...
  3. 외부 모듈인 경우 (import "github.com/park-jun-woo/fullend/<pkg>"):
...
| `github.com/park-jun-woo/fullend` | 기본 func 구현 (Go 모듈) |
