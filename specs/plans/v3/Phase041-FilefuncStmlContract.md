...
| stml/validator | 3 | 546 | 25 | 19 | ~44 | 284줄 |
...
- 테스트 파일도 연쇄 수정 필요 (parser_test.go 등)

## 실행 절차

1. 파일 분해 — F1/F2/F3 해소
2. `//ff:func`/`//ff:type`/`//ff:what` 부착 — A1/A3 해소
3. Q1 해소 (contract/hash, contract/scan, contract/splice, stml/parser, ssac/parser 등)
4. `go build ./...` + `go test ./...`
5. `filefunc validate` 패키지 단위 확인
