...
//fullend:gen ssot=service/schedule/get_schedule.ssac contract=e159c83
func (h *Handler) GetSchedule(c *gin.Context) {
	...
	if _, err = authz.Check(authz.CheckRequest{Action: "GetSchedule", Resource: "workflow", UserID: currentUser.ID, Role: currentUser.Role, ResourceID: wf.ID}); err != nil {
		c.JSON(http.StatusForbidden, gin.H{"error": "Not authorized"})
		return
	}

	result, err := schedule.GetSchedule(schedule.GetScheduleRequest{WorkflowID: wf.ID})
	if err != nil {
