...
func scanWorkflow(row interface{ Scan(...interface{}) error }) (*Workflow, error) {
	var w Workflow
	err := row.Scan(&w.ID, &w.OrgID, &w.Title, &w.TriggerEvent, &w.Status, &w.Version, &w.RootWorkflowID, &w.CreatedAt)
	if err != nil {
		return nil, err
	}
	return &w, nil
...
//fullend:gen ssot=db/workflows.sql contract=038bd84
func (m *workflowModelImpl) CreateVersion(orgID int64, title string, triggerEvent string, status string, version int64, rootWorkflowID int64) (*Workflow, error) {
	row := m.conn().QueryRowContext(context.Background(),
		"INSERT INTO workflows (org_id, title, trigger_event, status, version, root_workflow_id)\nVALUES ($1, $2, $3, $4, $5, $6)\nRETURNING *;",
		orgID, title, triggerEvent, status, version, rootWorkflowID)
	return scanWorkflow(row)
}
...
//fullend:gen ssot=db/workflows.sql contract=b3e6535
func (m *workflowModelImpl) FindByIDAndOrgID(id int64, orgID int64) (*Workflow, error) {
	row := m.conn().QueryRowContext(context.Background(),
...
//fullend:gen ssot=db/workflows.sql contract=9a871cb
func (m *workflowModelImpl) ListVersions(rootWorkflowID int64, orgID int64) ([]Workflow, error) {
	rows, err := m.conn().QueryContext(context.Background(),
		"SELECT * FROM workflows WHERE (root_workflow_id = $1 OR id = $1) AND org_id = $2 ORDER BY version DESC;",
		rootWorkflowID, orgID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
