Feature: Gig Lifecycle
  ...
  Scenario: Happy Path - Gig creation to fund release
    ...
    When POST CreateGig {"title": "Build API", "description": "REST API project", "budget": 1000} clientToken -> gigResult
    Then status == 200

    When PUT PublishGig {"id": gigResult.gig.id} clientToken
    Then status == 200
    ...
    When POST SubmitProposal {"id": gigResult.gig.id, "bid_amount": 900} freelancerToken -> proposalResult
    Then status == 200

    When POST AcceptProposal {"id": proposalResult.proposal.id} clientToken
    Then status == 200

    When POST SubmitWork {"id": gigResult.gig.id} freelancerToken
    Then status == 200

    When POST ApproveWork {"id": gigResult.gig.id} clientToken
    Then status == 200
  ...
  Scenario: Unauthorized - Freelancer B cannot submit work on Freelancer A's gig
    ...
    When POST CreateGig {"title": "Design Logo", "description": "Logo design", "budget": 500} clientToken2 -> gig2
    Then status == 200

    When PUT PublishGig {"id": gig2.gig.id} clientToken2
    Then status == 200
    ...
    When POST SubmitProposal {"id": gig2.gig.id, "bid_amount": 400} flTokenA -> proposal2
    Then status == 200

    When POST AcceptProposal {"id": proposal2.proposal.id} clientToken2
    Then status == 200
    ...
    When POST SubmitWork {"id": gig2.gig.id} flTokenB
    Then status == 403
  ...
  Scenario: Invalid State - Cannot approve work when gig is open
    ...
    When POST CreateGig {"title": "Write Docs", "description": "Documentation", "budget": 300} clientToken3 -> gig3
    Then status == 200

    When PUT PublishGig {"id": gig3.gig.id} clientToken3
    Then status == 200

    When POST ApproveWork {"id": gig3.gig.id} clientToken3
    Then status == 409
