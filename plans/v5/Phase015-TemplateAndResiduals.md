# ✅ Phase015 — TemplateAndResiduals (완료: 2026-04-14)
...
- [x] Part A: `main_template.go` + `query_opts_template.go` 를 `text/template` + `//go:embed` 로 전환. `templates/main.tmpl` + `templates/query_opts.tmpl` 추출. `go_templates.go` 는 기존부터 text/template 사용 중.
- [x] Part A: 생성 산출물 bit-level 동일성 확인 (diff exit 0 — gigbridge main.go, queryopts.go)
- [x] Part B: `pkg/ → internal/*` import **0 hit** (이전 7건 → 0). `internal/policy` → `pkg/parser/rego` 치환. `adapt_policies.go` 2 파일 삭제
- [x] Part C: `docs/filefunc-policy.md` 작성 — F1/F2 엄격 + test 면제 + A10~A13/Q1/Q4 엄격 정책 확정
- [x] `go build ./... && go vet ./... && go test ./pkg/...` 통과
- [x] `fullend gen dummys/{gigbridge,zenflow}/specs` 회귀 없음
- [x] `reports/metrics-phase015.md` 생성
- [x] `filefunc validate` baseline **36** (37 → 36, adapt_policies.go 2 삭제 효과)
- [ ] 커밋: `feat(generate): template 전환 + pkg 의존 독립 + filefunc 정책 확정 (Phase015)`

### 부산물

- `pkg/generate/gogin/templates/main.tmpl` + `templates/query_opts.tmpl`
- `pkg/generate/gogin/main_template_data.go` — MainTmplData struct
- `pkg/generate/gogin/main_template.go` / `query_opts_template.go` — text/template + embed 로 재작성
- `pkg/generate/hurl/adapt_policies.go`, `pkg/generate/gogin/adapt_policies.go` 삭제
- `pkg/generate/hurl/{build_operation_role_map, generate_hurl_tests}.go`, `pkg/generate/gogin/{build_ownerships_literal, generate_main, authz}.go` — `[]*policy.Policy` → `[]rego.Policy`
- `internal/orchestrator/{gen_authz, count_rego_policy_rules, run_codegen_steps}.go` — 상응 업데이트
- `docs/filefunc-policy.md` — 프로젝트 공식 filefunc 정책
- `reports/metrics-phase015.md`
