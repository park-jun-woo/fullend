...
func (h *Handler) ListWorkflows(c *gin.Context) {
	...
	if _, err := authz.Check(authz.CheckRequest{Action: "ListWorkflows", Resource: "workflow", UserID: currentUser.ID, Role: currentUser.Role, ResourceID: currentUser.ID}); err != nil {
		c.JSON(http.StatusForbidden, gin.H{"error": "Not authorized"})
		return
	}

	workflows, err := h.WorkflowModel.ListByOrgID(currentUser.OrgID)
	if err != nil {
