...
//fullend:gen ssot=service/proposal/submit_proposal.ssac contract=2677316
func (h *Handler) SubmitProposal(c *gin.Context) {
	...
	if _, err = authz.Check(authz.CheckRequest{Action: "SubmitProposal", Resource: "proposal", UserID: currentUser.ID, Role: currentUser.Role, ResourceID: currentUser.ID}); err != nil {
		c.JSON(http.StatusForbidden, gin.H{"error": "Not authorized"})
		return
	}

	proposal, err := h.ProposalModel.WithTx(tx).Create(gig.ID, currentUser.ID, bidAmount, "pending")
	if err != nil {
