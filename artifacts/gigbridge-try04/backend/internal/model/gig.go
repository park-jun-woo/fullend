...
//fullend:gen ssot=db/gigs.sql contract=76587cb
func (m *gigModelImpl) UpdateStatusAndFreelancer(id int64, status string, freelancerID int64) error {
	_, err := m.conn().ExecContext(context.Background(),
		"UPDATE gigs SET status = $2, freelancer_id = $3 WHERE id = $1;",
		id, status, freelancerID)
	return err
}
