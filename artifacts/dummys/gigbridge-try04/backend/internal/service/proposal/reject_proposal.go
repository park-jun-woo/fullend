...
//fullend:gen ssot=service/proposal/reject_proposal.ssac contract=2717bcc
func (h *Handler) RejectProposal(c *gin.Context) {
	...
	err = h.ProposalModel.WithTx(tx).UpdateStatus(proposal.ID, "rejected")
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Proposal 수정 실패"})
		return
	}

	updated, err := h.ProposalModel.WithTx(tx).FindByID(proposal.ID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Proposal 조회 실패"})
		return
	}
