...
//fullend:gen ssot=service/proposal/accept_proposal.ssac contract=d6a9392
func (h *Handler) AcceptProposal(c *gin.Context) {
	...
	err = h.GigModel.WithTx(tx).UpdateStatusAndFreelancer(gig.ID, "in_progress", proposal.FreelancerID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Gig 수정 실패"})
		return
	}

	r, err := billing.HoldEscrow(billing.HoldEscrowRequest{Amount: gig.Budget, ClientID: currentUser.ID, GigID: gig.ID})
	if err != nil {
