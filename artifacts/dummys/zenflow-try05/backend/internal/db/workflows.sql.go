...
const workflowListByOrg = `-- name: WorkflowListByOrg :many
SELECT id, org_id, title, trigger_event, status, created_at FROM workflows WHERE org_id = $1
`

func (q *Queries) WorkflowListByOrg(ctx context.Context, orgID int64) ([]Workflow, error) {
	rows, err := q.db.QueryContext(ctx, workflowListByOrg, orgID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
