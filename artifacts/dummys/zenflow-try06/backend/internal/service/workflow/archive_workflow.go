...
//fullend:gen ssot=service/workflow/archive_workflow.ssac contract=85c84e9
func (h *Handler) ArchiveWorkflow(c *gin.Context) {
	...
	if _, err = authz.Check(authz.CheckRequest{Action: "ArchiveWorkflow", Resource: "workflow", UserID: currentUser.ID, Role: currentUser.Role, ResourceID: wf.ID}); err != nil {
		c.JSON(http.StatusForbidden, gin.H{"error": "Not authorized"})
		return
	}

	if err := workflowstate.CanTransition(workflowstate.Input{Status: wf.Status}, "ArchiveWorkflow"); err != nil {
		c.JSON(http.StatusConflict, gin.H{"error": err.Error()})
		return
	}

	err = h.WorkflowModel.WithTx(tx).UpdateStatus(wf.ID, "archived")
	if err != nil {
