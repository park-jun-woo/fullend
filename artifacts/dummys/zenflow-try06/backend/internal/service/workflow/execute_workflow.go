...
//fullend:gen ssot=service/workflow/execute_workflow.ssac contract=5482b2a
func (h *Handler) ExecuteWorkflow(c *gin.Context) {
	...
	if _, err = authz.Check(authz.CheckRequest{Action: "ExecuteWorkflow", Resource: "workflow", UserID: currentUser.ID, Role: currentUser.Role, ResourceID: wf.ID}); err != nil {
	...
	execLog, err := h.ExecutionLogModel.WithTx(tx).Create(wf.ID, wf.OrgID, "completed", result.CreditsUsed)
	if err != nil {
