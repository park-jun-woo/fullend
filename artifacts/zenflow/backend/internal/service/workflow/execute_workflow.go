...
import (
	...
	"github.com/geul-org/zenflow/internal/billing"
	"github.com/geul-org/zenflow/internal/worker"
...
//fullend:gen ssot=service/workflow/execute_workflow.ssac contract=4e22544
func (h *Handler) ExecuteWorkflow(c *gin.Context) {
	...
	if _, err = worker.ProcessAction(worker.ProcessActionRequest{WorkflowID: wf.ID}); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "호출 실패"})
		return
	}

	dc, err := billing.DeductCredit(billing.DeductCreditRequest{OrgID: wf.OrgID})
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "호출 실패"})
		return
	}

	ex, err := h.ExecutionModel.WithTx(tx).Create(wf.ID, wf.OrgID, "completed", dc.CreditsDeducted)
	if err != nil {
