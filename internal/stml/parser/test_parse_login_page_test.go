...
func TestParseLoginPage(t *testing.T) {
  ...
  <div data-action="Login" class="space-y-4">
	...
	if page.Name != "login-page" { t.Errorf("Name = %q, want %q", page.Name, "login-page") }
	if len(page.Fetches) != 0 { t.Errorf("Fetches = %d, want 0", len(page.Fetches)) }
	if len(page.Actions) != 1 { t.Fatalf("Actions = %d, want 1", len(page.Actions)) }
	action := page.Actions[0]
	if action.OperationID != "Login" { t.Errorf("OperationID = %q, want %q", action.OperationID, "Login") }
	if len(action.Fields) != 2 { t.Fatalf("Fields = %d, want 2", len(action.Fields)) }
