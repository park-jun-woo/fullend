...
func fixImports(src string, modulePath string) string {
	...
	// Fix authz import: "authz" -> "github.com/park-jun-woo/ssac/pkg/authz"
	if strings.Contains(src, "\t\"authz\"\n") {
		src = strings.ReplaceAll(src, "\t\"authz\"\n", "\t\"github.com/park-jun-woo/ssac/pkg/authz\"\n")
	}

	// Fix queue import: "queue" -> "github.com/park-jun-woo/ssac/pkg/queue"
	if strings.Contains(src, "\t\"queue\"\n") {
		src = strings.ReplaceAll(src, "\t\"queue\"\n", "\t\"github.com/park-jun-woo/ssac/pkg/queue\"\n")
	}

	// Fix config import: "config" -> "github.com/park-jun-woo/ssac/pkg/config"
	if strings.Contains(src, "\t\"config\"\n") {
		src = strings.ReplaceAll(src, "\t\"config\"\n", "\t\"github.com/park-jun-woo/ssac/pkg/config\"\n")
	}

	// Fix auth import: ssac/pkg/auth -> project internal/auth (reexport.go bridges pkg/auth utilities)
	if strings.Contains(src, "\"github.com/park-jun-woo/ssac/pkg/auth\"") {
		src = strings.ReplaceAll(src, "\"github.com/park-jun-woo/ssac/pkg/auth\"",
